...
const exportExcel = (data, titles, workSheetColumnNames, workSheetName, filePath) => {
...
//const exportProductToExcel = (data, titles, workSheetColumnNames, workSheetName, filePath) => {
  //const data = products.map(product => {
  //    return [product.category, product.subcategory, product.productname,
  //        product.salesamount, product.freight, product.taxamount, product.extendedamount];
  //});
  //let data = Object.keys(dataobj).map(function(_) { return dataobj[_]; })
//  exportExcel(data, titles, workSheetColumnNames, workSheetName, filePath);
//}
//module.exports = exportProductToExcel;

const exportProductToExcel = (dataobj, titles, workSheetColumnNames, workSheetName, filePath) => {
  let data = []
  dataobj.forEach(datarow => {
    let row = []
    workSheetColumnNames.forEach(title => {
      if(datarow.hasOwnProperty(title)) row.push(datarow[title])
      else row.push("")
    });
    data.push(row)
  });
...
router.post("/reports/excel", async (req, res, next) => {
  ...
  try {
  let object = await  createExcel(req.body.content[0].reportjson)
	  //console.log(object);
  let workSheetColumnName = Object.keys(object[0])
   const name = makename(25)
   let titles = [];
   Object.values(req.body.content[0].reportjson.titles).forEach((element) => {
    //getTitleTable.push(element);
    // getTitleExport.push(element.title);
    titles.push([element.title])
  });
  let filePath = `public/uploads/${name}.xlsx`;
  await exportProductToExcel(object, titles, workSheetColumnName, name, filePath);
  res.json({name: name})
          ...
          if (item[pagebreackitem.rptData]) {
            if (pagebreackitem.SubGroup.SubPageBreak == "Y") {
              if ((newListData.length - 1) > numcheck) {
               console.log(config.Columns.length,titletable,columnslist)
                rowdata += `
                </tbody>
                </table>
